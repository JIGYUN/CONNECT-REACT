...
/** "YYYY-MM-DDTHH:mm" or "YYYY-MM-DD HH:mm(:ss)" → local Date (safe guards) */
function parseLocal(dt: string | null | undefined): Date | null {
  ...
  const mo = m[2];
  const d = m[3];
  if (!y || !mo || !d) return null; // noUncheckedIndexedAccess 대응
...
export default function ReservationListPage() {
  ...
  const list = (q.data ?? [])
    .slice()
    .sort((a, b) => getStartMillis(a) - getStartMillis(b));
  ...
  const matrix = useMemo(
    () => makeMonthMatrix(viewYear, viewMonth0),
    [viewYear, viewMonth0],
  );
  ...
  const submitQuick = async () => {
    ...
    // ❗ undefined 키를 아예 넣지 않기( exactOptionalPropertyTypes 대응 )
    const vars: ReservationInsertVars = {
      title: t,
      resvStartDt: startIso,
      resvEndDt: endIso,
      ...(grpCd ? { grpCd } : {}),
      ...(ownerId !== null ? { ownerId } : {}),
      resourceNm: resourceNm.trim() ? resourceNm.trim() : null,
      capacityCnt: capacityCnt, // number | null
      statusCd: 'SCHEDULED',
    };

    await mInsert.mutateAsync(vars);
  ...
  const toggleStatus = async (id: Id, checked: boolean) => {
    ...
    await mToggle.mutateAsync({
      reservationId: rid,
      statusCd: checked ? 'DONE' : 'SCHEDULED',
    });
  };
  ...
  return (
    <div className="rsv-root">
      <div className="header">
        <div className="nav">
          <button className="btn-icon" onClick={() => moveMonth(-1)} aria-label="이전 달">
            «
          </button>
          <div className="month">
            {viewYear}. {pad(viewMonth0 + 1)}
          </div>
          <button className="btn-icon" onClick={() => moveMonth(1)} aria-label="다음 달">
            »
          </button>
        </div>
        <div className="ctrl">
          <button className="pill" onClick={onToday}>
            오늘
          </button>
                ...
                {row.map((d, ci) => {
                  ...
                  return (
                    <td key={ci} onClick={() => onPick(d)} title={toYMD(d)}>
                      <div
                        className={`day ${sel ? 'is-sel' : ''} ${isToday(d) ? 'is-today' : ''} ${
                          inMonth ? '' : 'is-other'
                        }`}
                      >
                        {d.getDate()}
                ...
                return (
                  <tr key={String(id)} onClick={() => id !== null && goModify(id)}>
                    ...
                    <td style={{ textAlign: 'center' }}>
                      <span className={badgeClass(r.statusCd)}>
                        {(r.statusCd ?? 'SCHEDULED').toUpperCase()}
                      </span>
                    </td>
                    <td style={{ textAlign: 'center' }} onClick={(e) => e.stopPropagation()}>
                      <button className="btn-ghost" onClick={() => id !== null && void removeRow(id)}>
                        삭제
                      </button>
                    </td>
                  </tr>
