...
function TasksPage() {
    ...
    const listParams = useMemo(
        () => ({ dueDate: dateStr, grpCd, ownerId: OWNER_ID }),
        [dateStr, grpCd],
    );
    ...
    const toggleRow = async (id: UiTask['id'], checked: boolean) => {
        try {
            ...
            if (n == null) { alert('아직 저장되지 않은 항목입니다.'); return; }
        ...
        <div className="wrap mx-auto max-w-[720px] p-4">
            ...
            <div className="mb-4 rounded-xl bg-white p-3 shadow-sm">
                <div className="mb-2 flex items-center justify-between">
                    <button onClick={() => moveMonth(-1)} className="rounded-lg border border-slate-200 px-2 py-1 text-sm">‹</button>
                    <div className="text-sm font-medium">{viewY}.{pad(viewM0 + 1)}</div>
                    <button onClick={() => moveMonth(1)} className="rounded-lg border border-slate-200 px-2 py-1 text-sm">›</button>
                </div>
                ...
                <div className="grid grid-cols-7 gap-1 text-center">
                    {matrix.flat().map((d, i) => {
                        ...
                        const todayBadge = isToday ? 'after:absolute after:-top-1.5 after:right-1 after:h-1.5 after:w-1.5 after:rounded-full after:bg-black' : '';
                        return (
                            <div key={i} onClick={() => pick(d)} className={`${base} ${tone} ${sel} ${todayBadge} hover:bg-slate-50`}>
                                {d.getDate()}
                            </div>
                        );
                    })}
            ...
            <div className="mb-3 flex gap-2">
                <input ref={titleRef} type="text" placeholder="할 일을 입력"
                       className="flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm" />
                <input value={time} onChange={(e) => setTime(e.target.value)} type="time"
                       className="w-[110px] rounded-lg border border-slate-200 px-2 py-2 text-sm" />
                <button onClick={onAdd} className="rounded-lg bg-black px-3 py-2 text-sm text-white">추가</button>
            </div>
                        ...
                        {items.map((r) => (
                            <li key={r.id} className="flex items-center justify-between py-3">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button onClick={() => void deleteRow(r.id)} className="text-xs text-red-500">삭제</button>
                                </div>
                            </li>
                        ))}
                    </ul>
